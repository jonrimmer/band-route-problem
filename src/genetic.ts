...
const fitnessSort = (a: RouteItem, b: RouteItem) => a.fitness - b.fitness;
...
export const crossover = (
  ...
  const endIndex =
    startIndex + Math.ceil(Math.random() * (points.length - startIndex));
...
export function* genetic(points: Point[]): IterableIterator<Result> {
  ...
  for (let g = 1; g <= GENERATIONS; g++) {
    ...
    console.debug(
      `Worst route in population: ${population[population.length - 1].fitness}`
    );

    const offspring: RouteItem[] = [];
